...
const Nav: React.FC<NavProps> = ({override, gallery, work, onClickSettings, unlockGallery, unlockWork}) => {
    return (
        <div className="flex h-auto ml-0">
            <nav className="grid grid-cols-3 w-[95%] bg-navcolor rounded-lg m-1 ml-0">
                <h2 className="flex justify-center">
                    {(true) && <Link to="/luciensite/" className="link bg-navcolor rounded-sm text-[calc(1vh+1vw)] m-auto">--Home--</Link>}
                </h2>
                <h2 className="flex justify-center" onClick={() => {if(!gallery && !override){ unlockGallery();}}}>
                    {<Link to="/luciensite/gallery"  className={` ${gallery || override ? 'link':'link-disabled pointer-events-none select-none'} m-auto bg-contain drop-shadow-lg
                ...
                <h2 className="flex justify-center" onClick={() => {if(!work && !override){ unlockWork();}}}>
                    {<Link to="/luciensite/work-samples"  className={` ${work || override ? 'link':'link-disabled pointer-events-none select-none'} m-auto bg-cover drop-shadow-lg
